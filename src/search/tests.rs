use super::*;
use crate::{
    EngineCategory, EngineConfig, EngineFailure, EngineOutcomeKind, EngineOutput, Metrics,
    ProviderError, ProviderErrorKind, SearchImage, SearchReport, SearchResult,
};
...
impl Engine for RichEngine {
    ...
    async fn search_output(&self, query: &SearchQuery) -> Result<EngineOutput> {
        ...
        Ok(EngineOutput::new(results)
            ...
            .with_image(
                SearchImage::new("https://example.com/image.png").with_description("query image"),
            )
            .with_report(
...
async fn bulkhead_rejection_returns_half_open_probe_without_ejecting_upstream() {
    ...
    circuit.acquire("shared-engine").unwrap().record_failure(
        &EngineFailure::new("shared-engine", "provider_transport", "offline").with_transient(true),
    );
